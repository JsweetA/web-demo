...
setupMock({
  setup() {
    ...
    installMock(new RegExp('/api/content-period-analysis'), () => {
      const getLineData = (name) => {
        return {
          name,
    ...
    installMock(new RegExp('/api/content-publish'), () => {
    ...
    installMock(new RegExp('/api/popular-author/list'), () => {
      const generateData = () => {
        const list = new Array(7).fill(0).map((_item, index) => ({
          ranking: index + 1,
          author: installMock('@ctitle(5)'),
          contentCount: installMock(/[0-9]{4}/),
          clickCount: installMock(/[0-9]{4}/),
        }));
        return {
          list,
        };
